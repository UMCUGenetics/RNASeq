...
sub usage{
  ...
  Run by typing:	perl RNAseqAnalyse.pl -input [run directory] -outputDir [output directory]
    ...
    -mail                             [s] Email adress to send pipeline status mails to [string]
    ...
    -p|sgeProjectName                 [s] The project name used for sge qsub commands [string] Default : cog_bioinf
    -debug                            [s]	Debug mode
...
GetOptions (
  ...
  'p|sgeProjectName=s'  => \$opt{sgeProjectName},
  'mail=s'				=> \$opt{mail},
...
print SETTINGS "SGEPROJECTNAME=$opt{sgeProjectName}\n";
...
foreach my $sample (keys %{$samples}) {
  ...
  print MAPPING_SH "TMPDIR=$rundir/$sample/mapping/tmp/\n\n";
    ...
    my $R1_base = (split( /\./, basename($R1)))[0];
    my $R2_base = (split( /\./, basename($R2)))[0];
    ...
    if ($opt{fastqc} eq "yes") {
      ...
      print FASTQC "if [ -s $R1_base\_fastqc.html ]; then\n";
      ...
      print FASTQC "fi\n";
        ...
        print FASTQC2 "if [ -s $R2_base\_fastqc.html ]; then\n";
  ...
  # print MAPPING_SH "fi\n";
    ...
    print QSUB "##MAPPING\nqsub -P $opt{sgeProjectName} -l h_rt=08:00:00,h_vmem=34G -pe threaded $opt{nthreads} -o $rundir/$sample/logs -e $rundir/$sample/logs -R yes -N $job_id $rundir/$sample/jobs/$job_id.sh\n\n";
	}
    ...
    if ( $opt{id} eq "all" ){
  	    ...
  	    print QSUB  "##HTSeq-count\nqsub -P $opt{sgeProjectName} -l h_rt=08:00:00 -o $rundir/$sample/logs -e $rundir/$sample/logs -R yes -N $HTSeqCount_job_id -hold_jid $job_id $rundir/$sample/jobs/$HTSeqCount_job_id.sh\n\n";
	    }
	   }else{
...
if ( $opt{variantcalling} eq "yes" ){
  ...
  print VC "mkdir $rundir/tmp/\n\n";
  print VC "TMPDIR=$rundir/tmp/\n\n";
  #variant calling
...
if ( $opt{count} eq "yes" ){
  ...
  if ( $hold_line eq '' ){
    ...
    print QSUB "\n##Merge HTSeq-count tables\nqsub -P $opt{sgeProjectName} -l h_rt=02:00:00 -o $rundir/logs -e $rundir/logs -R yes -N $mergeTables_job_id -hold_jid $hold_line $rundir/jobs/$mergeTables_job_id.sh\n\n";
  }
...
if ( $opt{de} eq "yes"){
  ...
  my $hold_line = join ',',@hold_ids;
  if ( $hold_line eq '' ){
    print QSUB "\n##Perform DE analysis\nqsub -P $opt{sgeProjectName} -l h_rt=02:00:00 -o $rundir/logs -e $rundir/logs -R yes -N $de_job_id $rundir/jobs/$de_job_id.sh\n\n";
  } else {
...
if ( $opt{bamqc} eq "yes"){
  ...
  if ( $paired==0 ){
    print BQ "perl $opt{bamstats_path} -bam ".join(" -bam ",@final_bams)." -queue_threads 2 -queue_project $opt{sgeProjectName} -picard_path $opt{picard_path} -debug -rna -ref_flat $opt{refflat_file} -ribosomal_intervals $opt{intervallist} -strand $picard_strand -single_end -genome $opt{fasta} -run_name $runname -output_dir $rundir/bamMetrics\n\n";
  }
  elsif ( $paired==1 ){
  ...
  if ( $opt{mapping} eq "no" ){
    ...
    my $hold_line = join ',',@hold_mapping_ids;
    print QSUB "\n##bamQC\nqsub -P $opt{sgeProjectName} -l h_rt=02:00:00 -o $rundir/logs -e $rundir/logs -R yes -N $bamQC_job_id -hold_jid $hold_line $rundir/jobs/$bamQC_job_id.sh\n\n";
  }
